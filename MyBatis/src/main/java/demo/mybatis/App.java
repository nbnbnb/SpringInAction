...
import demo.mybatis.config.H2XMLConfig;
import demo.mybatis.config.SQLServerXMLConfig;
...
public class App {
    ...
    private static void sqlserverWithXml() {
        ApplicationContext context = new AnnotationConfigApplicationContext(SQLServerXMLConfig.class);
        BlogRepositoryXML blogRepository = context.getBean(BlogRepositoryXML.class);
        blogRepository.readBlog();
    }

    private static void h2WithXml() {
        ApplicationContext context = new AnnotationConfigApplicationContext(H2XMLConfig.class);
        BlogRepositoryXML blogRepository = context.getBean(BlogRepositoryXML.class);
        blogRepository.readBlog();
    }
}
